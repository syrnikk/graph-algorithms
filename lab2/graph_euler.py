...
def fleury_algorithm(graph: dict):
    ...
    def find_eulerian_trail(graph, u):
        for v in graph[u]:
            if not __is_bridge(graph, u, v):
                visited_vertices.append(u)
                __remove_edge(graph, u, v)
...
def __add_edge_back(graph: dict, u, v):
...
def __remove_edge(graph: dict, u, v):
...
def __is_bridge(graph: dict, u, v):
